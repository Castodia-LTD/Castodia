...
export default function CoreOrganisationHubPage({
  organisationId,
  children,
}: Props) {
  ...
  return (
    <CastodiaPageShell
      ...
      description="Manage this organisation's users, modules, timeline setup and CastodiaCore configuration."
      maxWidth="full"
    >
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <nav className="mt-8 flex flex-wrap gap-2 border-t border-slate-100 pt-5">
          {tabs.map((tab) => {
            ...
            const href = tab.path
              ? `/core/organisations/${organisation.id}/${tab.path}`
              : `/core/organisations/${organisation.id}`;
