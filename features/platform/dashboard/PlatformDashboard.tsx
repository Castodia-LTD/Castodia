...
function platformRoleLabel(role: PlatformUser["role"]) {
  return role === "castodia_owner"
    ? "Castodia Owner"
    : "Castodia Administrator";
}
...
export default function PlatformDashboardPage() {
  ...
  const [recentPlatformUsers, setRecentPlatformUsers] =
  ...
  async function loadDashboard() {
    ...
    try {
      ...
      if (!isPlatformUser) {
        throw new Error(
          "You do not have access to the Platform Dashboard."
        );
      }
      ...
      const [
        ...
        platformUsersCountResult,
        recentOrganisationsResult,
        recentPlatformUsersResult,
      ] = await Promise.all([
        supabase
          .from("organisations")
          .select("id", {
      ...
      const firstError =
        ...
        platformUsersCountResult.error ??
        recentOrganisationsResult.error ??
        recentPlatformUsersResult.error;
      ...
      setStats({
        ...
        platformUsers:
          platformUsersCountResult.count ?? 0,
      });
      ...
      setRecentPlatformUsers(
        (recentPlatformUsersResult.data ??
          []) as PlatformUser[]
      );
    } catch (error) {
      setLoadError(
  ...
  return (
    <CastodiaPageShell
      ...
      description="Monitor Castodia organisations, users and platform activity."
      maxWidth="wide"
    >
      {loadError ? (
        <CastodiaCard>
      ...
      <CastodiaSection
        ...
        description="Current totals across the Castodia platform."
      >
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        <CastodiaSection
          ...
          description="Recent Castodia platform accounts."
        >
          <CastodiaCard padding="none">
            {loading ? (
      ...
      <CastodiaSection
        title="Quick Actions"
        description="Common platform administration tasks."
      >
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <QuickAction
            href="/platform/organisations"
