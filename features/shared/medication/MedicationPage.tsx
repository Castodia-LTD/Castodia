...
import { MedicationList } from "@/components/shared/medication/MedicationList";
import ServiceUserHubHeader from "@/features/manager/service-users/components/ServiceUserHubHeader";

import { getMedicationProfiles } from "@/lib/service-user-hub/medication/api";
import { supabase } from "@/lib/supabase";

import type { HubMedicationRecord } from "@/lib/service-user-hub/medication/api";
...
function getErrorMessage(error: unknown) {
  return error instanceof Error
    ? error.message
    : "The medication information could not be loaded.";
}
...
export default function MedicationPage({
  ...
  const [loadedData, setLoadedData] =
    useState<LoadedMedicationPage | null>(null);
  ...
  const loadPage = useCallback(async () => {
    ...
    try {
      const [serviceUsersResult, medications] =
        await Promise.all([
          supabase
            .from("service_users")
            .select(`
          ...
          getMedicationProfiles(serviceUserId),
        ]);
  ...
  function handleServiceUserChange(
    ...
    router.push(
      `/${portal}/service-users/${nextServiceUserId}/medication`,
    );
  }
    ...
    return (
      <div className="flex min-h-64 flex-col items-center justify-center rounded-2xl border border-red-200 bg-red-50 px-6 text-center">
        ...
        <p className="mt-2 max-w-lg text-sm text-red-800">
          {errorMessage ||
            "The medication information could not be opened."}
        </p>
