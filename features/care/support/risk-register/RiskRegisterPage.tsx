...
import { RiskRegister } from "@/components/care/support/risk-register/RiskRegister";
import ServiceUserHubHeader from "@/features/care/manager/service-users/components/ServiceUserHubHeader";

import { getRiskRegister } from "@/lib/care/service-user-hub/risk-register/api";
...
type LoadedSupportRiskRegisterPage = {
...
export default function RiskRegisterPage() {
  ...
  const [loadedData, setLoadedData] =
    useState<LoadedSupportRiskRegisterPage | null>(null);
  ...
  const loadPage = useCallback(async () => {
    ...
    try {
      const [serviceUsersResult, assessments] = await Promise.all([
        supabase
          .from("service_users")
          .select(`
            id,
  ...
  function handleServiceUserChange(nextServiceUserId: string) {
    ...
    router.push(
      `/care/support/service-users/${nextServiceUserId}/risk-assessments`,
    );
  }
    ...
    return (
      <div className="flex min-h-64 flex-col items-center justify-center rounded-2xl border border-red-200 bg-red-50 px-6 text-center">
        ...
        <p className="mt-2 max-w-lg text-sm text-red-800">
          {errorMessage ||
            "The risk register could not be opened."}
        </p>
  ...
  return (
    <div className="space-y-6">
      ...
      <RiskRegister assessments={assessments} />
    </div>
  );
}