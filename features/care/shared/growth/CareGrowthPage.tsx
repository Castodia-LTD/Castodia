    ...
    <CastodiaPageShell title="Growth" description={`What matters now for ${overview.person.fullName}.`} maxWidth="wide">
      ...
      <section className="rounded-[28px] bg-gradient-to-br from-teal-700 via-cyan-700 to-teal-800 p-6 text-white shadow-lg sm:p-8">
        <div className="flex flex-wrap items-start justify-between gap-5">
          ...
          {portal === "manager" ? <CastodiaButton onClick={() => openGoal()} className="bg-white text-teal-800 hover:bg-cyan-50"><Plus size={17} /> Add goal</CastodiaButton> : null}
        </div>
      ...
      {portal === "manager" && editing ? <GoalEditor form={form} setForm={setForm} editing={editing} saving={saving} onCancel={() => setEditing(null)} onSave={() => void saveGoal()} /> : null}
    </CastodiaPageShell>
  );
}
