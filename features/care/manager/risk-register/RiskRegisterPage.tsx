...
import { RiskRegister } from "@/components/care/manager/risk-register/RiskRegister";
import ServiceUserHubHeader from "@/features/care/manager/service-users/components/ServiceUserHubHeader";

import { getCurrentRiskRegisterManager } from "@/lib/care/service-user-hub/risk-register/api";
...
export default function RiskRegisterPage() {
  ...
  const [loadedData, setLoadedData] =
    useState<LoadedRiskRegisterPage | null>(null);
  ...
  const loadPage = useCallback(async () => {
    ...
    try {
      const [manager, serviceUsersResult] = await Promise.all([
        getCurrentRiskRegisterManager(),
  ...
  function handleServiceUserChange(nextServiceUserId: string) {
    ...
    router.push(
      `/care/manager/service-users/${nextServiceUserId}/risk-assessments`,
    );
  }
