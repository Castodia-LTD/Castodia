...
import { CASE_CATEGORIES, CASE_RISK_LEVELS } from "@/lib/safeguarding/constants";
import type {
  CaseCategory,
  CaseRiskLevel,
  ServiceUserOption,
...
export default function NewSafeguardingCaseForm() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.push(`/manager/safeguarding/${caseId}`);
      router.refresh();
    } catch (saveError) {
    ...
    <div className="mx-auto w-full max-w-5xl space-y-6 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-3">
          <Link href="/manager/safeguarding" className="inline-flex min-h-10 items-center rounded-xl px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100">
            Cancel
          </Link>
